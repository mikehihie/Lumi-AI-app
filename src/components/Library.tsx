...
import { BookOpen, Upload, FileText, Play, Loader2, CheckCircle2, XCircle } from 'lucide-react';
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      {showUpload && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 mb-6">
          ...
          <input type="text" placeholder={t('docName', state.language)} value={uploadName} onChange={e => setUploadName(e.target.value)} className="w-full px-4 py-2 border dark:border-slate-600 dark:bg-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none mb-4" />
          <textarea placeholder={t('pasteContent', state.language)} value={uploadText} onChange={e => setUploadText(e.target.value)} className="w-full px-4 py-2 border dark:border-slate-600 dark:bg-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none mb-4 min-h-[150px]" />
          <div className="flex justify-end gap-2">
            ...
            <button onClick={handleUpload} className="px-4 py-2 bg-indigo-600 text-white rounded-xl font-medium">{t('saveDoc', state.language)}</button>
          </div>
        </motion.div>
      )}
